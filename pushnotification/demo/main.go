...
import (
	...
	"github.com/gioui-plugins/gio-plugins/pushnotification/giopushnotification"
...
func init() {
	giopushnotification.DefaultProviders = []pushnotification.ExternalConfig{
		pushnotification.AndroidFirebaseConfig{
			AppID:     "1:1051400967388:android:7de4b2fc688d7a07199019",
			ProjectID: "giodemo-6fc3a",
	...
	for {
		...
		case app.FrameEvent:
			...
			if ButtonAction.Clicked(gtx) || submitted {
				gioplugins.Execute(gtx, giopushnotification.RequestTokenCmd{Tag: w})
			}

			for {
				evt, ok := gioplugins.Event(gtx, giopushnotification.Filter{Target: w})
				if !ok {
					break
				}

				switch evt := evt.(type) {
				case giopushnotification.TokenReceivedEvent:
					log.Printf("FIREBASE Token: %s\n", evt.Token)
					InputAction.SetText(evt.Token)
				case giopushnotification.ErrorEvent:
					InputAction.SetText("Error: " + evt.Error.Error())
					log.Printf("Error: %s\n", evt.Error)
				}
			}
