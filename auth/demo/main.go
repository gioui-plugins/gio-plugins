		...
		for {
			...
			case app.FrameEvent:
				...
				if buttonGoogle.Clicked(gtx) {
					gtx.Execute(gioauth.OpenCmd{Provider: google.IdentifierGoogle, Nonce: nonce()})
				}

				if buttonApple.Clicked(gtx) {
					gtx.Execute(gioauth.OpenCmd{Provider: apple.IdentifierApple, Nonce: nonce()})
				}
				...
				for {
					evt, ok := gtx.Event(gioauth.Filter{})
					if !ok {
						break
					}
...
func init() {
	gioauth.DefaultProviders = []providers.Provider{
		&google.Provider{
			WebClientID:     "413620002498-sv2sftp4fk3ttkdr9uampsi03eg5c66h.apps.googleusercontent.com",
			DesktopClientID: "413620002498-30v79n5m1f715fgr89df8c646a3gliu5.apps.googleusercontent.com",
